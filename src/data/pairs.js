...
export const fetchDailyGainers = async () => {
  ...
  try {
    const response = await axios.get(
      "https://open-api.dextools.io/free/v2/ranking/ether/gainers",
      options
    );
...
export const fetchDailyLosers = async () => {
  ...
  try {
    const response = await axios.get(
      "https://open-api.dextools.io/free/v2/ranking/ether/losers",
      options
    );
    const formattedData = response.data.data.map((item) => ({
      mainTokenSymbol: item.mainToken.symbol,
...
export const fetchDailyHotPairs = async () => {
  ...
  try {
    const response = await axios.get(
      "https://open-api.dextools.io/free/v2/ranking/ether/hotpools",
      options
    );
    const formattedData = response.data.data.map((item) => ({
      mainTokenSymbol: item.mainToken.symbol,
