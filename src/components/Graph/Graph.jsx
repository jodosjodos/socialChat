...
import { HistoricalChart, SingleToken } from "../config/api";
...
const CoinInfo = ({ token }) => {
  const [tokenInfo, setCoinInfo] = useState({});
  ...
  const fetchHistoricData = async () => {
    tokenDetails = await axios.get(SingleToken(token && token.id));
    setCoinInfo({
      market_cap: tokenDetails.data.market_data.market_cap.usd,
      supply: tokenDetails.total_supply,
    });

    const { data } = await axios.get(
      HistoricalChart(token && token.id, days, currency)
    );
    setflag(true);
      ...
      historicData.map((token, index) => {
        ...
        return (
          ...
          >
            {isHovered?.id === index && isHovered.hovered && (
              <div className="flex flex-col gap-8 bg-white min-w-max border-4 rounded-2xl p-5 absolute bottom-28 z-[1000] dark:bg-[#202020] dark:border-[#333333]">
                {usersWithVotes.map((userWithVote) => (
                  <div
                    key={userWithVote.id}
                    className="flex flex-row  w-fit items-center gap-[13px]"
                  >
                    <div>
                      <img
                        src={userWithVote.profile}
                        alt="user profile"
                    ...
                    <div>
                      ...
                      <p className="text-[#898989] semi-bold text-[15px]">
                        {userWithVote.msg}
                      </p>
                    </div>
                  </div>
            ...
            <p className="text-[#898989] font-bold lg:text-[10px] md:text-[8px] text-[6px]">
              {label}
            </p>
          </div>
        );
    ...
    <div>
      {!historicData || flag === false ? (
        <div className="w-full">
          <img
            src={
              isDarkTheme ? "/images/tradeGraph.png" : "/images/whiteGraph.png"
            }
            className=" blur-md"
            alt=""
          />
        ...
        <>
          <Line
            ...
            data={{
              labels: historicData.map((token, index) => {
                let date = new Date(token[0]);
                let time =
                  date.getHours() > 12
            ...
            options={{
              ...
              onHover: (event, chart) => {
                ...
                const dataPoint = historicData.find(
                  (data) => data[0] === xValue
                );
          ...
          <div>
            ...
            <div className="flex flex-row items-center md:gap-5 justify-center">
              <p className="text-[#898989] font-semibold md:text-2xl text-xl">
                Highlights
              </p>
